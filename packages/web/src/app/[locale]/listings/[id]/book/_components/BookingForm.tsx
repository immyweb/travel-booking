  ...
  const schema = useMemo(
    () =>
      ClientCreateBookingSchema.refine((data) => data.guests <= maxGuests, {
        message: t('maxGuestsError', { count: maxGuests }),
        path: ['guests'],
      }),
    [maxGuests, t],
  );
      ...
      >
        ...
        {hasDateRange ? (
          ...
          <div className="flex items-end gap-2">
            <div className="flex flex-col gap-1">
              <Label htmlFor="book-check-in">{t('checkIn')}</Label>
              <Input id="book-check-in" type="date" className="w-40" {...register('checkIn')} />
            </div>
            <div className="flex flex-col gap-1">
              <Label htmlFor="book-check-out">{t('checkOut')}</Label>
        ...
        {guests ? (
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm">{t('guestsCount', { count: guests })}</p>
            <input type="hidden" {...register('guests', { valueAsNumber: true })} />
          </div>
        ) : (
          <div className="flex flex-col gap-1">
            <Label htmlFor="book-guests">{t('guests')}</Label>
            <Input
              id="book-guests"
              type="number"
              min={1}
        ...
        <div className="flex flex-col gap-1">
          <Label htmlFor="book-guest-name">{t('fullName')}</Label>
          <Input id="book-guest-name" type="text" {...register('guestName')} />
        </div>

        <div className="flex flex-col gap-1">
          <Label htmlFor="book-guest-email">{t('email')}</Label>
