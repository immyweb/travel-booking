...
function PaymentForm({ bookingId, clientSecret }: { bookingId: string; clientSecret: string }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (confirmError) {
      // confirmError.message is Stripe's own (already-localized) decline
      // message — only the fallback for a message-less error is ours to
      // translate.
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm text-muted-foreground">{t('testCardHint')}</p>
