import { ListingDetailSchema, ListingQuerySchema } from '@travel-booking/core';
import { Router } from 'express';
import { z } from 'zod';
import type { Db } from '../../db/db';
import { ApiError } from '../../errors/errors';
import { validateParamsAndQuery } from '../../errors/validate';
import { getListingById } from './listings.service';

// Malformed (non-UUID) ids are a client mistake (400); well-formed ids that
// match no row are a missing resource (404) — validateParamsAndQuery only
// owns the first distinction.
...
export function createListingsRouter(db: Db): Router {
  ...
  listingsRouter.get(
    ...
    validateParamsAndQuery(
      ListingParamsSchema,
      ListingQuerySchema,
      async ({ id }, { checkIn, checkOut }, _req, res) => {
